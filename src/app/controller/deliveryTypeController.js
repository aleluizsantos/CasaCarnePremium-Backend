...
router.get('/:id', async (req, res) => {
    const { id } = req.params;
    const deliveryType = await connection('deliveryType').where("id", id).select("*");
...
router.delete('/:id', async (req, res) => {
    const { id } = req.params;
    try {
        const delDelivery = await connection('deliveryType').where('id', id).delete();

        return res.json({ Message: delDelivery ? "Exclusão concluída com sucesso" : "Nenhuma exclusão foi realizada" });
